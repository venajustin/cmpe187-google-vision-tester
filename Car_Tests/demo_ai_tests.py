...
Runs all 19 AI test cases and displays for each:
- Input: Shows the input image first
- Expected Output: Expected number of vehicles + expected bounding boxes (blue)
...
def show_input_image(image_path, test_id, description, expected_count):
    ...
    ax.set_title(f"Input: {test_id}\n{description}\nExpected: {expected_count} vehicles", fontsize=10, fontweight='bold')
    ax.axis('off')
...
def show_before_after(input_image_path, output_image_path, test_id, description, expected, detected, count_passed, loc_info=None, expected_boxes=None):
    ...
    if expected_boxes:
        for i, box_data in enumerate(expected_boxes):
            ...
            vehicle_type = box_data.get('type', 'Vehicle')
            label = f"Expected {i+1}: {vehicle_type}"
    ...
    if loc_info:
        ...
        title = f"Output: Detected: {detected} vehicles | Count: {count_status}\n{loc_status}"
    else:
        title = f"Output: Detected: {detected} vehicles\n{count_status}"
...
def run_test(test_id, client, images_dir, output_dir, expected_localizations=None):
    ...
    if expected_boxes is not None:
        matched_count, avg_iou, loc_details = calculate_localization_score(expected_boxes, vehicles_detected)
        loc_info = {
    ...
    json_data = create_json_output(test_id, vehicles_detected, output_files, timing_info, expected_count, count_passed)
    ...
    for i, test_id in enumerate(test_ids, 1):
        ...
        if result['all_objects']:
            ...
            for name, score in result['all_objects']:
                is_vehicle = name.lower() in ['car', 'truck', 'vehicle', 'bus']
        ...
        if result['vehicle_details']:
            ...
            for j, (name, score) in enumerate(result['vehicle_details'], 1):
                print(f"    Vehicle {j} ({name}): {score*100:.1f}% confidence")
