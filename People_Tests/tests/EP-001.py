...
Description: Severe Backlighting/Silhouette Class. People standing with the sun/bright light directly behind, creating a silhouette, with minimal features visible.

Test Input:
[IMAGE: People silhouetted against bright sun in urban street]

Input Categories:
- Environmental Conditions: Severe backlighting (silhouette)
- Distance Range: Close to Medium
- Occlusion Level: None (100% visible)
- Group Size: Small Group (2 people)

Expected Result:
- 2 people detection successful
- Lower confidence acceptable (0.6-0.8) due to silhouetting
- Shape-based detection (relying on body contours/outlines)
- Bounding boxes may be less precise at edges

Success Criteria:
- PASS: ≥85% detection rate, count within ±1 tolerance, confidence 0.6-0.8
...
def test_ep_001():
    """
    Test EP-001: Severe Backlighting/Silhouette Class

    This test verifies detection with severe backlighting creating silhouettes.
    """
    TEST_ID = 'EP-001'
    ...
    print(f"\nTEST CASE EP-001: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Severe Backlighting/Silhouette Class',
        'category': 'Equivalence Partition',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Severe backlighting (silhouette)',
        ...
        'test_reason': (
            ...
            f"Meets functional requirements for Severe Backlighting/Silhouette Class."
            if test_passed else
