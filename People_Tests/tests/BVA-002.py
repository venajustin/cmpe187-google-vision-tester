"""
Test Case ID: BVA-002
Category: Boundary Value Analysis
Description: Single Person Detection (1 vs 0 boundary). One person in normal
attire crossing the street in perfect daylight, with no obstacles. Establishes
performance ceiling and baseline for all other tests.

Test Input:
[IMAGE: Single person crossing at crosswalk in clear daylight]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close (facial features visible)
...
- Bounding box: Reasonably positioned (visual assessment)
- No false positives or false negatives

Success Criteria:
- PASS: ≥85% detection rate (1 person detected), count within tolerance, proper bounding box
- FAIL: <85% detection rate (0 people detected), count error exceeds tolerance
...
def test_bva_002():
    """
    Test BVA-002: Single Person Detection (1 vs 0 boundary)

    This test verifies that the Google Vision API correctly detects
    a single person with maximum confidence in ideal conditions.
    """
    TEST_ID = 'BVA-002'
    ...
    # Calculate metrics
    actual_people = 1  # Ground truth: 1 person in scene
    ...
    print(f"\nTEST CASE BVA-002: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Single Person Detection (1 vs 0 boundary)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight',
            'distance_range': 'Close (facial features visible)',
        ...
        'test_reason': (
            f"Test PASSED: Expected 1 person, detected {detected_people} people. "
            ...
            f"Meets functional requirements despite {'' if detected_people == 1 else 'not having '}exact match."
            if test_passed else
            f"Test FAILED: Expected 1 person, detected {detected_people} people. "
