...
Description: Adverse Weather Conditions (Heavy Snow). Urban street with 3 pedestrians in active heavy snowfall, reduced visibility (~50%).

Test Input:
[IMAGE: Three people walking in heavy snowfall on sidewalk]

Input Categories:
- Environmental Conditions: Heavy snow (reduced visibility)
- Distance Range: Close to Medium
- Occlusion Level: Weather-related (snowfall)
- Group Size: Small Group (3 people)

Expected Result:
- Detect 3 pedestrians (count = 3, tolerance: ±1 acceptable)
- Lower confidence acceptable (0.5-0.7) due to weather
- No false positives from falling snow particles
- All visible individuals detected despite snow

Success Criteria:
- PASS: ≥60% detection rate (weather challenge), count within ±1 tolerance
...
def test_ep_009():
    """
    Test EP-009: Adverse Weather Conditions (Heavy Snow)

    This test verifies detection in heavy snow conditions.
    """
    TEST_ID = 'EP-009'
    ...
    print(f"\nTEST CASE EP-009: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Adverse Weather Conditions (Heavy Snow)',
        'category': 'Equivalence Partition',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Heavy snow (reduced visibility)',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Weather-related (snowfall)',
        ...
        'test_reason': (
            ...
            f"Meets functional requirements for Adverse Weather Conditions (Heavy Snow)."
            if test_passed else
