"""
Test Case ID: BVA-010
Category: Boundary Value Analysis
Description: Count of 10 Pedestrians (Medium Group Maximum). Ten people in a busy urban area with typical crowd conditions, some occlusions.

Test Input:
[IMAGE: Ten people walking on sidewalk with backlighting from sun]

Input Categories:
- Environmental Conditions: Daylight (backlighting)
- Distance Range: Close to Medium
- Occlusion Level: Moderate (crowd + backlighting)
- Group Size: Medium Group (10 people - maximum)

Expected Result:
- Detection count = 10
- Detection rate: ≥85% (at least 9/10 detected)
- Count accuracy: Within ±2 tolerance for medium group
- Medium group classification (boundary case)
- Confidence >0.65 (backlighting and occlusions present)

Success Criteria:
- PASS: ≥85% detection rate, count within tolerance, confidence >0.65
...
def test_bva_010():
    """
    Test BVA-010: Count of 10 Pedestrians (Medium Group Maximum)

    This test verifies detection of 10 people (medium group maximum) with backlighting.
    """
    TEST_ID = 'BVA-010'
    ...
    print(f"\nTEST CASE BVA-010: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Count of 10 Pedestrians (Medium Group Maximum)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight (backlighting)',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Moderate (crowd + backlighting)',
            'group_size': 'Medium Group (10 people - maximum)'
        },
        'expected_results': {
            'actual_people_in_scene': 10,
        ...
        'comparison': {
            ...
            'expected_vs_actual_count': {
                ...
                'exact_match': detected_people == 10
            },
            'criteria_checks': {
                'detection_rate': {
                    'threshold': 85,
        ...
        'test_reason': (
            f"Test PASSED: Expected 10 people, detected {detected_people} people. "
            f"Detection rate: {metrics['detection_rate']:.1f}% (≥85% required). "
            f"Count error: {metrics['count_error']} (within ±{metrics['count_tolerance']} tolerance for medium groups). "
            f"Meets functional requirements."
            if test_passed else
            f"Test FAILED: Expected 10 people, detected {detected_people} people. "
