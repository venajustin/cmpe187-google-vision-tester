...
Description: Weather + Visibility + Pedestrian Detection
Detailed Description: A person with an umbrella crossing in rain, wet pavement with active rainfall, and vehicles present.

Test Input:
    [IMAGE: DT-007.jpg from images folder]

Expected Result:
- ENHANCED SENSITIVITY mode activated
- Detect 1 pedestrian crossing
- Medium confidence acceptable (0.55-0.75)
- HIGH alert (elevated risk)
- EXTENDED STOPPING alert (weather-aware safety)
- Active rainfall recognized
- Wet pavement conditions identified

Pass Criteria:
    Detect 1 person (±1 tolerance), detection rate ≥60% (weather conditions), HIGH alert with ENHANCED SENSITIVITY mode
...
def test_dt_007():
    ...
    test_config = {
        ...
        'category': 'Decision Table',
        'description': 'Weather + Visibility + Pedestrian Detection',
        'detailed_description': 'A person with an umbrella crossing in rain, wet pavement with active rainfall, and vehicles present.',
        'actual_people': actual_people,
        'detection_threshold': detection_threshold,
        'group_size_category': group_size_category,
        'expected_results': """- ENHANCED SENSITIVITY mode activated
- Detect 1 pedestrian crossing
- Medium confidence acceptable (0.55-0.75)
- HIGH alert (elevated risk)
- EXTENDED STOPPING alert (weather-aware safety)
- Active rainfall recognized
- Wet pavement conditions identified""",
        'pass_criteria': 'Detect 1 person (±1 tolerance), detection rate ≥60% (weather conditions), HIGH alert with ENHANCED SENSITIVITY mode'
    }
