"""
Test Case ID: BVA-021
Category: Boundary Value Analysis
Description: Group at Night (4-5 people boundary in low light). Group of 4-5 people under street lighting at night.

Test Input:
[IMAGE: Four people walking at night under street lights]

Input Categories:
- Environmental Conditions: Night (street lighting)
...
Expected Result:
- Detection count = 4
- Detection rate: ≥60% (low light conditions)
- Count accuracy: Within ±1 tolerance for small group
- Lower confidence acceptable (0.6-0.8) due to nighttime conditions
- Small group classification (4-5 boundary)
...
def test_bva_021():
    """
    Test BVA-021: Group at Night (4-5 people boundary in low light)

    This test verifies detection in low light conditions at night.
    """
    TEST_ID = 'BVA-021'
    ...
    print(f"\nTEST CASE BVA-021: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Group at Night (4-5 people boundary in low light)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Night (street lighting)',
