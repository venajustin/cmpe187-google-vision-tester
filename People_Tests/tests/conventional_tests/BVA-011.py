"""
Test Case ID: BVA-011
Category: Boundary Value Analysis
Description: Exact Count of 11 Pedestrians (Large Group Minimum). Eleven people in a crowd scene with dense conditions, overlapping expected.

Test Input:
[IMAGE: Eleven people walking in formation, artistic atmospheric photo]

Input Categories:
- Environmental Conditions: Daylight (atmospheric)
- Distance Range: Close to Medium
- Occlusion Level: Dense (overlapping expected)
- Group Size: Large Group (11 people - minimum)

Expected Result:
- Detection count = 11
- Detection rate: ≥85% (at least 10/11 detected)
- Count accuracy: Within ±±20% (9-13) tolerance for large group
- Large group classification (critical boundary)
- Confidence >0.60 (dense conditions with overlapping expected)

Success Criteria:
- PASS: ≥85% detection rate, count within tolerance, confidence >0.60
...
def test_bva_011():
    """
    Test BVA-011: Exact Count of 11 Pedestrians (Large Group Minimum)

    This test verifies detection of 11 people (large group minimum) with dense conditions.
    """
    TEST_ID = 'BVA-011'
    ...
    print(f"\nTEST CASE BVA-011: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Exact Count of 11 Pedestrians (Large Group Minimum)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight (atmospheric)',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Dense (overlapping expected)',
            'group_size': 'Large Group (11 people - minimum)'
        },
        'expected_results': {
            'actual_people_in_scene': 11,
        ...
        'comparison': {
            ...
            'expected_vs_actual_count': {
                ...
                'exact_match': detected_people == 11
            },
            'criteria_checks': {
                'detection_rate': {
                    'threshold': 85,
        ...
        'test_reason': (
            f"Test PASSED: Expected 11 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 11 people, detected {detected_people} people. "
