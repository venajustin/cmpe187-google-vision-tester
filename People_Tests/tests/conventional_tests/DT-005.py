...
Description: Empty Street + Urban + Wet + Night Conditions
Detailed Description: Urban street at night after rain with no people present, wet conditions, low light with street lighting only.

Test Input:
    [IMAGE: DT-005.jpg from images folder]

Expected Result:
- No detection required (count = 0)
- MONITOR mode
- LOW alert level (lowest priority)
- No false positives from wet surface reflections
- System correctly identifies empty scene

Pass Criteria:
    Detect 0 people, no false positives from wet reflections or shadows
...
def test_dt_005():
    ...
    group_size_category = 'individual'
    ...
    test_config = {
        ...
        'category': 'Decision Table',
        'description': 'Empty Street + Urban + Wet + Night Conditions',
        'detailed_description': 'Urban street at night after rain with no people present, wet conditions, low light with street lighting only.',
        'actual_people': actual_people,
        'detection_threshold': detection_threshold,
        'group_size_category': group_size_category,
        'expected_results': """- No detection required (count = 0)
- MONITOR mode
- LOW alert level (lowest priority)
- No false positives from wet surface reflections
- System correctly identifies empty scene""",
        'pass_criteria': 'Detect 0 people, no false positives from wet reflections or shadows'
    }
