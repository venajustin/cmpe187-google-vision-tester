"""
Test Case ID: BVA-006
Category: Boundary Value Analysis
Description: Exact Count of 5 Pedestrians (Small Group Maximum). Five people walking on sidewalk with some depth of field blur present. Tests both counting accuracy and detection under moderate visual challenges.

Test Input:
[IMAGE: Five people on sidewalk with some out-of-focus in background]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Far (mixed)
- Occlusion Level: None (100% visible)
- Group Size: Small Group (5 people)

Expected Result:
- Detection count = 5
- Detection rate: ≥85% (at least 5/5 detected)
- Count accuracy: Within ±1 tolerance for small group
- Five bounding boxes
- Confidence >0.70 for background figures (due to blur)
- Confidence >0.80 for sharper foreground figures
...
def test_bva_006():
    """
    Test BVA-006: Exact Count of 5 Pedestrians (Small Group Maximum)

    This test verifies detection of 5 people with depth of field blur.
    """
    TEST_ID = 'BVA-006'
    ...
    print(f"\nTEST CASE BVA-006: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Exact Count of 5 Pedestrians (Small Group Maximum)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight',
            'distance_range': 'Close to Far (mixed)',
            'occlusion_level': 'None (100% visible)',
            'group_size': 'Small Group (5 people)'
        },
        'expected_results': {
            'actual_people_in_scene': 5,
        ...
        'test_reason': (
            f"Test PASSED: Expected 5 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 5 people, detected {detected_people} people. "
