...
Description: Night + Separated Path + Traffic
Detailed Description: Person on sidewalk at night with vehicles on road, low light conditions.

Test Input:
    [IMAGE: DT-003.jpg from images folder]

Expected Result:
- ENHANCED processing activated
- Detection with lower confidence (0.5-0.7)
- HIGH alert
- System recognizes complex/high-risk scenario
- Adaptive algorithms engaged for nighttime + traffic

Pass Criteria:
    Detect 1 person (±1 tolerance), detection rate ≥60% (nighttime + traffic), HIGH alert for complex scenario
...
def test_dt_003():
    ...
    test_config = {
        ...
        'category': 'Decision Table',
        'description': 'Night + Separated Path + Traffic',
        'detailed_description': 'Person on sidewalk at night with vehicles on road, low light conditions.',
        'actual_people': actual_people,
        'detection_threshold': detection_threshold,
        'group_size_category': group_size_category,
        'expected_results': """- ENHANCED processing activated
- Detection with lower confidence (0.5-0.7)
- HIGH alert
- System recognizes complex/high-risk scenario
- Adaptive algorithms engaged for nighttime + traffic""",
        'pass_criteria': 'Detect 1 person (±1 tolerance), detection rate ≥60% (nighttime + traffic), HIGH alert for complex scenario'
    }
