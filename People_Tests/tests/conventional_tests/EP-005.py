...
Description: Motion Blur Interference Class. Fast-moving person or camera movement causing blur, the person is still recognizable as human.

Test Input:
[IMAGE: Runner in motion with visible blur on legs/arms]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: None (motion blur)
- Group Size: Individual (1 person)

Expected Result:
- Detection attempted (not guaranteed)
- Lower confidence acceptable (0.5-0.7) due to motion blur
- Less precise bounding box (may include motion trails)
- Detection based on clearer body parts (torso/upper body)

Success Criteria:
- PASS: ≥60% detection rate (motion blur challenge), count within ±1 tolerance
...
def test_ep_005():
    """
    Test EP-005: Motion Blur Interference Class

    This test verifies detection with motion blur interference.
    """
    TEST_ID = 'EP-005'
    ...
    print(f"\nTEST CASE EP-005: {status_message}\n")
    ...
    test_config = {
        ...
        'test_reason': (
            ...
            f"Meets functional requirements for Motion Blur Interference Class."
            if test_passed else
