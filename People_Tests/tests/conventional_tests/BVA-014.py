"""
Test Case ID: BVA-014
Category: Boundary Value Analysis
Description: Count of 20 Pedestrians (Large Group Maximum). Twenty people in a very crowded scene with a dense crowd.

Test Input:
[IMAGE: Twenty people crossing street in dense formation]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Dense crowd (moderate occlusions)
- Group Size: Large Group (20 people - maximum)

Expected Result:
- Detection count = 20
- Detection rate: ≥85% (at least 17/20 detected)
- Count accuracy: Within ±±20% (16-24) tolerance for large group
- Large group classification (maximum threshold)
- Confidence >0.60 (dense crowd with moderate occlusions)
...
def test_bva_014():
    """
    Test BVA-014: Count of 20 Pedestrians (Large Group Maximum)

    This test verifies detection of 20 people (large group maximum) in dense crowd.
    """
    TEST_ID = 'BVA-014'
    ...
    print(f"\nTEST CASE BVA-014: {status_message}\n")
    ...
    test_config = {
        ...
        'comparison': {
            ...
            'expected_vs_actual_count': {
                ...
                'exact_match': detected_people == 20
            },
            'criteria_checks': {
                'detection_rate': {
                    'threshold': 85,
        ...
        'test_reason': (
            f"Test PASSED: Expected 20 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 20 people, detected {detected_people} people. "
