"""
Test Case ID: BVA-005
Category: Boundary Value Analysis
Description: Exact Count of 4 Pedestrians (Small Group Max-1). Four people at a sidewalk with all subjects visible, minimal occlusion.

Test Input:
[IMAGE: Four people walking in a line on urban sidewalk]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Minimal (<10% occlusion)
- Group Size: Small Group (4 people)

Expected Result:
- Detection count = 4
- Detection rate: 100% (4/4 detected)
- Count accuracy: Within ±1 tolerance for small group
- Four distinct bounding boxes
...
def test_bva_005():
    """
    Test BVA-005: Exact Count of 4 Pedestrians (Small Group Max-1)

    This test verifies detection of 4 people with minimal occlusion.
    """
    TEST_ID = 'BVA-005'
    ...
    print(f"\nTEST CASE BVA-005: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Exact Count of 4 Pedestrians (Small Group Max-1)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Minimal (<10% occlusion)',
