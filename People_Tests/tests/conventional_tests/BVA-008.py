"""
Test Case ID: BVA-008
Category: Boundary Value Analysis
Description: Exact Count of 7 Pedestrians (Medium Group Min+1). Seven people in a public space with mixed visibility, some partial occlusions are acceptable.

Test Input:
[IMAGE: Seven people standing in conversational groups outdoors]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Partial (>50% visible)
- Group Size: Medium Group (7 people)

Expected Result:
- Detection count = 7
- Detection rate: ≥85% (at least 6/7 detected)
- Count accuracy: Within ±2 tolerance for medium group
- Seven bounding boxes
- Medium group classification
- Confidence >0.70 (varied due to partial occlusions and mixed visibility)
...
def test_bva_008():
    """
    Test BVA-008: Exact Count of 7 Pedestrians (Medium Group Min+1)

    This test verifies detection of 7 people with partial occlusions.
    """
    TEST_ID = 'BVA-008'
    ...
    print(f"\nTEST CASE BVA-008: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Exact Count of 7 Pedestrians (Medium Group Min+1)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Partial (>50% visible)',
            'group_size': 'Medium Group (7 people)'
        },
        'expected_results': {
            'actual_people_in_scene': 7,
        ...
        'test_reason': (
            f"Test PASSED: Expected 7 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 7 people, detected {detected_people} people. "
