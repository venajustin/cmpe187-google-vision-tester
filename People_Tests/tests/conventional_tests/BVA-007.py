"""
Test Case ID: BVA-007
Category: Boundary Value Analysis
Description: Exact Count of 6 Pedestrians (Medium Group Minimum). Six people in a public space with all subjects visible, possible minor occlusions are acceptable.

Test Input:
[IMAGE: Six people at crosswalk/street crossing]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Minor (>90% visible)
- Group Size: Medium Group (6 people)

Expected Result:
- Detection count = 6
- Detection rate: ≥85% (at least 6/6 detected)
- Count accuracy: Within ±2 tolerance for medium group
- Six bounding boxes
- Confidence >0.75 for each detection
...
def test_bva_007():
    """
    Test BVA-007: Exact Count of 6 Pedestrians (Medium Group Minimum)

    This test verifies detection of 6 people (medium group minimum).
    """
    TEST_ID = 'BVA-007'
    ...
    print(f"\nTEST CASE BVA-007: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Exact Count of 6 Pedestrians (Medium Group Minimum)',
        'category': 'Boundary Value Analysis',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Daylight',
            'distance_range': 'Close to Medium',
            'occlusion_level': 'Minor (>90% visible)',
            'group_size': 'Medium Group (6 people)'
        },
        'expected_results': {
            'actual_people_in_scene': 6,
        ...
        'test_reason': (
            f"Test PASSED: Expected 6 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 6 people, detected {detected_people} people. "
