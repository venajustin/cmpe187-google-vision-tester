...
Description: Nighttime/Low-Light Conditions Class. A person walking on a dimly lit street at night with minimal ambient lighting, street lights only.

Test Input:
[IMAGE: Person walking on bridge/walkway at night with street lights]

Input Categories:
- Environmental Conditions: Night (street lighting only)
- Distance Range: Close to Medium
- Occlusion Level: None (100% visible)
- Group Size: Individual (1 person)

Expected Result:
- Detection attempted with reduced confidence (0.5-0.7)
- Single person detection (count = 1)
- Bounding box may be less precise due to poor lighting

Success Criteria:
- PASS: ≥60% detection rate (low light challenge), count within ±1 tolerance
...
def test_ep_002():
    """
    Test EP-002: Nighttime/Low-Light Conditions Class

    This test verifies detection in nighttime/low-light conditions with street lighting.
    """
    TEST_ID = 'EP-002'
    ...
    print(f"\nTEST CASE EP-002: {status_message}\n")
    ...
    test_config = {
        'test_id': TEST_ID,
        'test_name': 'Nighttime/Low-Light Conditions Class',
        'category': 'Equivalence Partition',
        'actual_people': actual_people,
        'input_categories': {
            'environmental_conditions': 'Night (street lighting only)',
        ...
        'test_reason': (
            ...
            f"Meets functional requirements for Nighttime/Low-Light Conditions Class."
            if test_passed else
