"""
Test Case ID: BVA-017
Category: Boundary Value Analysis
Description: 25% Occlusion (Minor Obstruction). Person 25% hidden behind an object (pole, tree) with 75% visible, clear lighting.

Test Input:
[IMAGE: Person partially obscured by foreground bokeh/branches]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Minor (25% occluded, 75% visible)
...
- Successful detection (person detected despite 25% occlusion)
- Confidence >0.75 (minor occlusion, majority visible)
- Accurate bounding box encompassing visible portions
...
def test_bva_017():
    """
    Test BVA-017: 25% Occlusion (Minor Obstruction)

    This test verifies detection with 25% occlusion (minor obstruction).
    """
    TEST_ID = 'BVA-017'
    ...
    print(f"\nTEST CASE BVA-017: {status_message}\n")
