"""
Test Case ID: BVA-009
Category: Boundary Value Analysis
Description: Count of 9 Pedestrians (Medium Group Max-1). Nine people in a crowd formation, with some occlusion expected in group settings.

Test Input:
[IMAGE: Nine people crossing street at crosswalk]

Input Categories:
- Environmental Conditions: Daylight
- Distance Range: Close to Medium
- Occlusion Level: Moderate (crowd density)
- Group Size: Medium Group (9 people)

Expected Result:
- Detection count = 9
- Detection rate: ≥85% (at least 8/9 detected)
- Count accuracy: Within ±2 tolerance for medium group
- Nine bounding boxes
- Medium group classification
- Confidence >0.65 (lower threshold due to crowd density and occlusions)
...
def test_bva_009():
    """
    Test BVA-009: Count of 9 Pedestrians (Medium Group Max-1)

    This test verifies detection of 9 people in crowd formation with occlusions.
    """
    TEST_ID = 'BVA-009'
    ...
    print(f"\nTEST CASE BVA-009: {status_message}\n")
    ...
    test_config = {
        ...
        'test_reason': (
            f"Test PASSED: Expected 9 people, detected {detected_people} people. "
            ...
            if test_passed else
            f"Test FAILED: Expected 9 people, detected {detected_people} people. "
