...
Description: Pedestrian Crossing + Vehicle Moving
Detailed Description: Person on crosswalk with vehicle approaching, clear visibility, active traffic.

Test Input:
    [IMAGE: DT-004.jpg from images folder]

Expected Result:
- CRITICAL priority alert (HIGHEST in system)
- Immediate detection required (minimize latency)
- ACTIVE safety warning generated immediately
- EMERGENCY scenario classification
- Detection confidence: >0.85 (good visibility)

Pass Criteria:
    Detect 1 person (±1 tolerance), detection rate ≥85%, CRITICAL priority for active crossing with approaching vehicle
...
def test_dt_004():
    ...
    test_config = {
        ...
        'category': 'Decision Table',
        'description': 'Pedestrian Crossing + Vehicle Moving',
        'detailed_description': 'Person on crosswalk with vehicle approaching, clear visibility, active traffic.',
        'actual_people': actual_people,
        'detection_threshold': detection_threshold,
        'group_size_category': group_size_category,
        'expected_results': """- CRITICAL priority alert (HIGHEST in system)
- Immediate detection required (minimize latency)
- ACTIVE safety warning generated immediately
- EMERGENCY scenario classification
- Detection confidence: >0.85 (good visibility)""",
        'pass_criteria': 'Detect 1 person (±1 tolerance), detection rate ≥85%, CRITICAL priority for active crossing with approaching vehicle'
    }
